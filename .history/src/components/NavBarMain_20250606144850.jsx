...
function NavbarMain() {
  ...
  const { currentUser, logout, cartItems, uniqueItemCount } = useAuth();
  ...
  // دالة التمرير مع تأثير الانيميشن
  const scrollToSection = (id) => {
    const element = document.getElementById(id);
    if (!element) return;

    const navbarHeight = 70; // تعديل ارتفاع النافبار حسب تصميمك
    const start = window.scrollY;
    const end = element.getBoundingClientRect().top + window.scrollY - navbarHeight;
  ...
  // تمرير مع التحقق من المسار الحالي
